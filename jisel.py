...
def upsert_to_trivia_leader_board(discord_id, discord_name, score):
    ...
    try:
        with db.connect() as conn:
            participants = []
            leaderboard_table = Table('triviaLeaderboard', metadata, autoload=True, autoload_with=conn)
            update_or_insert_charge_query = f"INSERT INTO pwm.\"triviaLeaderboard\" (\"discord_id\", \"discord_name\", \"score\", \"lastUpdated\") VALUES ({discord_id}, \'{discord_name}\', {score}, {datetime.datetime.now()}) ON CONFLICT (\"discord_id\") DO UPDATE SET \"score\" = \"triviaLeaderboard\".\"score\" + {score}, \"lastUpdated\" = {datetime.datetime.now()}"
            result = conn.execute(update_or_insert_charge_query)
            select_st = select([leaderboard_table]).order_by(leaderboard_table.c.score.desc(), leaderboard_table.c.lastUpdated)
            res = conn.execute(select_st)
            for _row in res:
